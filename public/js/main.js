...
function emptyGrid() {
  ...
  for (let r = 0; r < CONFIG.GRID_ROWS; r++) {
    const row = [];
    for (let c = 0; c < CONFIG.GRID_COLS; c++) row.push({ alive: false, flash: null, a: 0 });
    tiles.push(row);
  }
  return tiles;
}
  ...
  if (res.mode === 'sensor') {
    ...
    setTimeout(() => {
      if (!input.hasSensorData) {
        flashHint(menuHint, 'Tilt blocked — open the https:// link the server printed.');
      }
    }, 1500);
  } else if (res.reason === 'denied') {
    flashHint(menuHint, 'Motion permission denied — using keyboard if available.');
...
net.on('state', (s) => {
  ...
  for (const id of [...ballsById.keys()]) if (!seen.has(id)) ballsById.delete(id);

  const me = s.balls.find((b) => b.id === net.youId);
  if (me) amAlive = me.a === 1;

  // Spectate banner only while a match is live and you're out.
  const live = s.phase !== PHASE.GAMEOVER;
  spectateBanner.classList.toggle('hidden', !(live && !amAlive));
  ...
  if (s.phase === PHASE.PREVIEW) {
    const frac = Math.max(0, Math.min(1, s.timer / Math.max(0.001, previewDuration(s.round))));
...
// Game over — show the final standings.
net.on('game:over', ({ winnerId, winnerName, rounds, standings }) => {
...
(function offerSecureLink() {
  ...
  menuHint.textContent = 'For tilt controls, tap the green button above (secure link).';
})();
...
function fullGrid() {
  ...
  for (let r = 0; r < CONFIG.GRID_ROWS; r++) {
    const row = [];
    for (let c = 0; c < CONFIG.GRID_COLS; c++) row.push({ alive: true, flash: null, a: 1 });
    tiles.push(row);
  }
  return tiles;
}
...
tuneBtn.addEventListener('click', async () => {
  ...
  practice.ball = {
    x: CONFIG.GRID_COLS / 2, y: CONFIG.GRID_ROWS / 2, vx: 0, vy: 0,
    color: CONFIG.COLORS.self, label: 'You', self: true, alive: true, trail: [],
  };
...
function frame(now) {
  ...
  if (practice.active) {
    ...
    const steps = 2;
    for (let i = 0; i < steps; i++) integrate(practice.ball, input.get(), dt / steps, phys);
